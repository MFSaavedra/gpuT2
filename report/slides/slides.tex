...
\begin{frame}{Arquitectura: un front-end sobre el mismo núcleo}
    \begin{columns}[T]
        \begin{column}{0.47\textwidth}
            ...
            \vspace{0.1cm}
            \begin{block}{Patrón}
                \emph{Strategy}: motor y camino de presentación intercambiables;
                el shader no cambia.
            \end{block}
        \end{column}
        \begin{column}{0.52\textwidth}
            \centering
            \includegraphics[height=0.80\textheight]{gui_architecture.png}
        \end{column}
    \end{columns}
\end{frame}
...
% ============================================================ 8. Qt
\begin{frame}{Qt: la ventana, el loop y los controles}
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            \footnotesize
        ...
        \begin{column}{0.46\textwidth}
            ...
            \end{block}
            \vspace{-0.15cm}
            {\scriptsize\ttfamily connect(playBtn, toggled,\\
            \hspace*{1em}view, \&GolGlWidget::setPlaying);}
...
\begin{frame}{De la Tarea 2 a la Tarea 3: qué se entregó y qué cambió}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{block}{Tarea 2 --- lo entregado}
                ...
                \end{itemize}
                Métrica: \textbf{celdas/s}; barridos de \texttt{block}/shared + perfilado
                Nsight. \alert{$\sim$35--37\,Gcel/s}, $\sim$240$\times$ sobre la CPU.
            \end{block}
        \end{column}
        \begin{column}{0.5\textwidth}
    ...
    \begin{highlightbox}
        \footnotesize
        \textbf{La clave:} el patrón \emph{Strategy} de la Tarea~2 (motores
        intercambiables tras \texttt{ISimEngine}) hizo que sumar un visor en tiempo
        real fuera casi todo \textbf{aditivo} --- la simulación no se modificó.
    \end{highlightbox}
\end{frame}
