...
\begin{sourcecode}{plaintext}{The strategy layout. Parenthesised backends/renderers are scaffolded or planned.}
...
gol_core  (Grid, Config+CLI, patterns, RLE, interfaces)
   ...
   '-- (gol_opencl -- opt-in, not yet present)
gol  executable  =  gol_core + gol_cpu + gol_render (+ gol_cuda when built)
\end{sourcecode}
...
\paragraph{Pending.} \texttt{OpenCLEngine} (\texttt{src/engines/opencl/}), its
cross-backend equivalence test, and its \texttt{std::chrono} profiling --- the only
remaining backend.
