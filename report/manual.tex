...
\begin{abstractd}
	...
	oracle; the CUDA backend is also complete (verified bit-for-bit against it).
		The OpenCL backend is scaffolded but not yet
...
\texttt{scripts/sweep.sh} (the CPU-threads and CUDA block\,$\times$\,shared sweeps,
with an adaptive-repeats runtime guard), and \texttt{analysis/results.ipynb} turns
the resulting \texttt{results/*.csv} into the report's figures. The speed-up is
...
adding a barrier and dropping L1 reuse --- it runs $\approx 0.55\times$ the global
kernel. Explaining \emph{why} is one of the more interesting parts of the report.
...
\texttt{wall}/\texttt{kernel} efficiency falls (to $\approx 40\%$ at $N=64$),
...
equivalence (Section \ref{sec:cuda}). The CPU engine is the reference oracle. The
benchmark sweeps (\texttt{scripts/sweep.sh}), the results CSVs, the analysis notebook
...
\paragraph{Pending.} All three backends are implemented and verified against the
CPU oracle, including the OpenCL cross-backend gtest
(\texttt{gol\_opencl\_tests}). What remains is mostly reporting: folding the
OpenCL command-queue timing into the report's profiling section alongside the
CUDA Nsight results.
