...
one of five colour ramps (grayscale by default, plus phosphor, amber, magma, ice).
The age value
...
uniform int   uPalette;        // 0 phosphor .. 4 ice (2 grayscale is the default)

vec3 palette(int p, float t);  // t in [0,1] -> RGB ramp (five palettes)
...
amber / magma / ice). A per-frame \texttt{statsChanged} signal updates the generation/FPS/zoom
