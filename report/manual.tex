...
though storage is row-major. The live-cell glyph defaults to the Unicode full
block (U+2588) and the glyph fields are \texttt{std::string} rather than
\texttt{char} so the multi-byte UTF-8 block fits; it is one display column wide,
so the grid stays aligned with the one-column \texttt{.} dead cell. This renderer
is for eyeballing correctness only --- never in a benchmark.
...
void AnsiRenderer::render(const Grid& grid, std::uint64_t generation) {
  ...
  const std::size_t visCols = std::min(grid.cols(), termCols);   // clip to the viewport
  std::string out;
...
$\min(\text{rows},\,\text{term}-1)\times\min(\text{cols},\,\text{term})$ window,
annotating the header, e.g. \texttt{generation 0 [80x23 of 300x300]}. The size is
re-queried every frame, so resizing the terminal is handled.
...
separately from host\,$\leftrightarrow$\,device transfers, and the speed-up is
...
parallel in one path), both renderers, the RLE pipeline, the CLI, and the first
three test suites. The CPU engine is the reference oracle.

\paragraph{Pending.} \texttt{CudaEngine} (\texttt{src/engines/cuda/}) and
\texttt{OpenCLEngine} (\texttt{src/engines/opencl/}); the cross-backend
equivalence test; and the benchmark sweeps, results CSVs, and analysis notebooks
that populate \texttt{main.tex}.
