...
\begin{table}[H]
	...
	\begin{tabular}{@{}lll@{}}
		...
		\texttt{include/gol/render/TextRenderer.hpp} & ASCII renderer declaration & \ref{sec:render} \\
...
\section{Rendering --- \texttt{NullRenderer} and \texttt{TextRenderer}}
\label{sec:render}
...
void TextRenderer::render(const Grid& grid, std::uint64_t generation) {
  std::string out;
  out.reserve(grid.size() + grid.rows() + 32);   // cells + one '\n' per row + header
...
though storage is row-major. This renderer is for eyeballing correctness only ---
