...
\paragraph{The fourth test: cross-backend equivalence.} \emph{Done for CUDA,
pending for OpenCL.} \texttt{cuda\_equivalence\_test.cpp} seeds CUDA and the
\texttt{CpuEngine} oracle identically and asserts bit-for-bit equality across block
sizes $\{32,64,128,256\}$, both kernels (global/shared) and both edge modes; OpenCL
will follow the same pattern. An
...
\paragraph{Pending.} \texttt{OpenCLEngine} (\texttt{src/engines/opencl/}) is now
implemented and verified at runtime against the CPU oracle
(\texttt{--engine opencl --verify}). What remains is a dedicated
\texttt{gol\_opencl\_tests} cross-backend gtest (mirroring the CUDA one) and
folding its command-queue timing into the report's OpenCL profiling section.
