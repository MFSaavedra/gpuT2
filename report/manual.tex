...
\texttt{IRenderer} --- the two strategy interfaces. \texttt{CpuEngine} realizes
\texttt{ISimEngine} today; \texttt{CudaEngine} and \texttt{OpenCLEngine} are
marked \emph{planned}. \texttt{NullRenderer}, \texttt{TextRenderer} and
\texttt{AnsiRenderer} realize \texttt{IRenderer}. Dashed arrows are dependencies
(e.g. \texttt{CpuEngine} uses \texttt{LifeRules::nextState} and \texttt{Timer};
...
two-block glyph); it annotates the header, e.g. \texttt{generation 0 [40x23 of
300x300]} ($300$ columns in an $80$-column terminal leaves room for $40$
