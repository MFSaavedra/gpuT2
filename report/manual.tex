...
\texttt{HybridEngine} runs the \emph{CPU and a GPU on the same board at the same
...
by \textbf{static load balancing} following the divisible-load-theory (DLT)
approach of Barlas, \emph{Multicore and GPU Programming} (2e), \S11.3.
...
\emph{almost} independent --- they interact only across the cut, where each side
...
example, and it is what makes the load \emph{divisible}: the per-generation
communication is a single row each way, independent of how many rows each side
owns.

We give each block a one-row \textbf{ghost} (halo) above and below its real rows,
...
device buffers. Each side ping-pongs \emph{locally} across generations, and the
...
\textbf{reads vertical neighbours straight from the ghost rows} (no vertical edge
logic at all) and applies only the \emph{horizontal} (column) edge rule. The host
fills the ghosts to realise whichever board edge mode is in force:

\begin{itemize}
  \item \textbf{Seam} (always): the CPU's last real row $\to$ the GPU's top ghost,
        and the GPU's first real row $\to$ the CPU's bottom ghost.
  \item \textbf{Far edges, bounded:} the outermost ghosts stay zero (dead). Both
        device/host buffers are zeroed once at upload and the kernel never writes
        ghost rows, so this costs nothing per step.
  \item \textbf{Far edges, toroidal} (\texttt{--wrap}): the board's top wraps to
...
\noindent Because the only difference between the two edge modes is \emph{which
...
partition where \emph{all nodes finish together}; for two nodes that gives
...
\begin{itemize}
  \item \textbf{\texttt{--cpu-frac F}} sets $\text{part}_{\text{cpu}}=F$ directly ---
        ...
        pure-CPU ends).
  \item \textbf{Calibration (default)} is the a-priori ``initialization phase'' of
...
i.e.\ roughly the \emph{maximum} of the two nodes' times --- which is what the
...
\emph{global-memory} halo kernel only --- the shared/local-memory option is studied
...
\emph{about} the pure-GPU rate rather than beating it --- the regime \S11.3
...
\emph{age heatmap} that colours a live cell by how many generations it has survived
...
\emph{spectrum}). The last is aimed at the age heatmap: unlike the others, which vary
\emph{brightness} between a dark and a light end (so on a single-hue ramp like phosphor
or grayscale young and old cells differ only in shade and are hard to tell apart), it
rotates \emph{hue} at fixed brightness --- red\,($t{=}0$, new) through green to
...
updates it \textbf{once per generation} (\texttt{age = alive ? min(age+1,255) : 0}),
...
\textbf{only while age mode is active} --- the binary and neighbour modes, and the
...
the cursor. The \textbf{P} key saves a PNG of the current view to the working
...
including a \textbf{Screenshot} button, a \emph{Colour} combo (binary / neighbour
count / age heatmap) and a \emph{Palette} combo (grayscale by default, plus phosphor /
...
\emph{and} an \texttt{if(EXISTS <source>)} check, so passing
...
sequential-vs-parallel, RLE loader, the cross-backend equivalence of \emph{both}
