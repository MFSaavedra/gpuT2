...
\def\documentsubtitle {Hybrid CPU+GPU load balancing: results and a verdict}
...
\subsection{OpenCL regressed to the pathological ICD path (environment, not code)}
\label{sec:opencl}

This Linux box previously measured OpenCL at $\sim$37\,Gcells/s, on par with CUDA
(both lower through the same PTX/SASS on NVIDIA). This run measured 1.70\,Gcells/s ---
reproducible on a single clean \texttt{--engine opencl} run, $\sim$14.7$\times$ below
CUDA. The \texttt{--engine opencl} code is unchanged on this branch, the GPU is
healthy for CUDA, and the slowdown is OpenCL-specific, so this is the driver/ICD
pathology previously seen for Windows, now affecting this Linux session. The action
item: the OpenCL rows in this sweep are not representative; reboot and re-run before
regenerating any committed OpenCL figures or trusting the OpenCL column. The hybrid
sweep used \texttt{--gpu-backend cuda}, so it is unaffected.
